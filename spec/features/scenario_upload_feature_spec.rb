...
RSpec.feature "ScenarioImport", type: :feature do
  ...
  def upload_scenario_json
    ...
    attach_file('Option 2: Upload a Scenario JSON File', Rails.root.join('spec/data/ifttt-roundtrip-scenario.json'))
  ...
  it 'does not allow to upload an invalid scenario' do
    visit new_scenario_import_path
    attach_file('Option 2: Upload a Scenario JSON File', Rails.root.join('spec/data/agents.json'))
    click_on 'Start Import'
    expect(page).to have_text('Please provide either a Scenario JSON File or a Public Scenario URL.')
  end
  ...
  context 'creating new scenarios' do
    ...
    it 'allows to create a scenario using an URL' do
      stub_request(:get, "http://example.com/scenario.json").
        ...
        to_return(status: 200, body: File.open(Rails.root.join('spec/data/ifttt-roundtrip-scenario.json')).read, headers: {})
      visit new_scenario_import_path
  ...
  context 'updating existing scenarios' do
    it 'shows the changed options', js: true do
      upload_scenario_json
      visit new_scenario_import_path
      attach_file('Option 2: Upload a Scenario JSON File', Rails.root.join('spec/data/ifttt-roundtrip-changed-scenario.json'))
      click_on 'Start Import'
